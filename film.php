...
<?php
	...
	$actor = mysqli_query($connect, "SELECT * FROM `actor` WHERE `actor_id` in(SELECT actor_id FROM `film_actor` WHERE `film_id` = '$film')");
	$num2 = mysqli_num_rows($actor);
        if($num){
                $arr = array();
	...
	if($num2){
                $arr2 = array();
                while($rs = mysqli_fetch_assoc($actor)){ $arr2[]=$rs;  }
        }
	$i=0;
                $id = $arr[$i]['film_id'];
...
<?php
        ...
        if($num){
		    ...
		    for($i=0;$i<count($arr3);$i++){
				...
				$result2 = mysqli_query($connect, "SELECT `nick_name` FROM `user_list` WHERE `user_id`='$user_id' ");
				$arr4 = array();
