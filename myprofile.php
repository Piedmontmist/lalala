...
<body>
	...
	<div class="login">
		<center>
			...
			<?php
			$sql = "SELECT * FROM `user_theme` WHERE `user_id` = '{$arr[0]["user_id"]}'  and `theme_id`=2";
			$re = mysqli_query($connect, $sql);
			$num = mysqli_num_rows($re);
			if ($num) $s_t1 = "checked";
			else $s_t1 = "";
			$sql = "SELECT * FROM `user_theme` WHERE `user_id` = '{$arr[0]["user_id"]}' and `theme_id` = 1";
			$re = mysqli_query($connect, $sql);
			$num = mysqli_num_rows($re);
			if ($num) $s_t2 = "checked";
			else $s_t2 = "";
			$sql = "SELECT * FROM `user_theme` WHERE `user_id` ='{$arr[0]["user_id"]}' and `theme_id` = 3";
			$re = mysqli_query($connect, $sql);
			$num = mysqli_num_rows($re);
			if ($num) $s_t3 = "checked";
			else $s_t3 = "";

			$sql = "SELECT * FROM `user_director` WHERE `user_id` ='{$arr[0]["user_id"]}' and `director_id` = 1";
			$re = mysqli_query($connect, $sql);
			$num = mysqli_num_rows($re);
			if ($num) $s_d1 = "checked";
			else $s_d1 = "";

			$sql = "SELECT * FROM `user_actor` WHERE `user_id` ='{$arr[0]["user_id"]}' and `actor_id` = 2";
			$re = mysqli_query($connect, $sql);
			$num = mysqli_num_rows($re);
			if ($num) $s_a1 = "checked";
			else $s_a1 = "";
			...
			<div class="login">
				...
				if (isset($_POST["submit"]) && $_POST["submit"] == "选择") {
					/*   theme  */
					mysqli_query($connect, "DELETE FROM `user_theme` WHERE `user_id` = '{$x}'");
					foreach ($_POST['check1'] as $val) {
						mysqli_query($connect, "INSERT INTO `user_theme` (`user_id`, `theme_id`) VALUES ('{$x}', '{$val}')");
					}
					...
					/*  director  */
					mysqli_query($connect, "DELETE FROM `user_director` WHERE `user_id` = '{$x}'");
					foreach ($_POST['check2'] as $val) {
						mysqli_query($connect, "INSERT INTO `user_director` (`user_id`, `director_id`) VALUES ('{$x}', '{$val}')");
					}
					/*  actor  */
					mysqli_query($connect, "DELETE FROM `user_actor` WHERE `user_id` = '{$x}'");
					foreach ($_POST['check3'] as $val) {
						mysqli_query($connect, "INSERT INTO `user_actor` (`user_id`, `actor_id`) VALUES ('{$x}', '{$val}')");
					}
				}
				//if($_POST)
